...
def plot_images(imgs, targets, paths=None, fname='images.jpg', image_channels=3):
    ...
    for i in range(bs):
        ...
        boxes[[1, 3]] *= h
        if image_channels == 1:
            plt.subplot(ns, ns, i + 1).imshow(imgs[i][0,:,:].transpose(0,1),cmap='gray') #grayscale
        else:
            plt.subplot(ns, ns, i + 1).imshow(imgs[i].transpose(1, 2, 0)) #BGR
