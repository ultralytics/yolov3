...
def quan_w_perchannel(w_kernel,kernel_bitwidth,out_channels):
    # w_transform = conv.weight.data.contiguous().view(conv.out_channels, -1)
    w_transform = w_kernel.data.contiguous().view(out_channels, -1)
    w_min = w_transform.min(dim=1).values
    w_max = w_transform.max(dim=1).values
    max_abs = torch.max(torch.stack([w_min.abs(), w_max.abs()], dim=1), dim=1).values
    w_kernel_radix_list = []
    for j in range(len(max_abs)):
        w_kernel_radix = getradix(max_abs[j], kernel_bitwidth)
        w_kernel_radix_list.append(w_kernel_radix)
    for c in range(w_kernel.shape[0]):
        with torch.no_grad():
            w_kernel[c, :, :, :] = torch.round(w_kernel[c, :, :, :] * 2.0 ** w_kernel_radix_list[c])/2.0 ** w_kernel_radix_list[c]
    return w_kernel
...
class QuanConv(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, quan_name_w='dorefa', quan_name_a='dorefa', nbit_w=10,
                 nbit_a=32, stride=1,
        ...
        name_w_dict = {'dorefa': quan_w_perchannel}
    ...
    def forward(self, input):
        ...
        if self.nbit_w <=32:
            #量化卷积
            w = self.quan_w(self.weight, self.nbit_w,self.out_channels)
        else:
            #卷积保持不变
            w = self.weight
