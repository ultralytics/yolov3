...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    """Trains a segmentation model using the provided hyperparameters, options, and callbacks, handling multi-GPU setups, data loading, logging, and validation."""
    (
        save_dir,
        epochs,
        batch_size,
...
def main(opt, callbacks=Callbacks()):
    """Initializes training or evolution of models with given options and callbacks, handling device setup and data preparation."""
    if RANK in {-1, 0}:
        print_args(vars(opt))
        check_git_status()
...
def run(**kwargs):
    """Executes model training with specified configurations; see example: `train.run(data='coco128.yaml', imgsz=320, weights='yolov5m.pt')`."""
    opt = parse_opt(True)
