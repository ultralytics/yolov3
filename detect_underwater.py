...
#     print('names:',names)
    ...
    for path, img, im0s, vid_cap in dataset:
        image_id = path.split('/')[-1].replace('jpg', 'xml')
...
#                         print(image_id, label, conf, c1, c2, c3, c4)
                        Label.append(label)
                        ID.append(image_id)
                        Conf.append(conf)
                        Xmin.append(xmin)
...
# #                             file.write(('%g ' * 6 + '\n') % (*xyxy, cls, conf))
#                             file.write(image_id + ' ' + label +  ' ' + str(int(conf)) +  ' ' + str(c1) +  ' ' + str(c2) +  ' ' + str(c3) +  ' ' + str(c4) + '\n')
# 打开下面注释可以可视化图片
...
if __name__ == '__main__':
    ...
    parser.add_argument('--names', type=str, default='data/underwater.names', help='*.names path')
    ...
    dataframe = pd.DataFrame({'name': Label, 'image_id': ID, 'confidence': Conf, 'xmin': Xmin, 'ymin': Ymin, 'xmax': Xmax, 'ymax': Ymax})
