    ...
    def forward(self, x):
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor, torch.Tensor]: A tuple containing three elements:
                - xywh (torch.Tensor): Tensor of shape [batch_size, num_detections, 4] containing normalized x, y, 
                  width, and height coordinates.
                - conf (torch.Tensor): Tensor of shape [batch_size, num_detections, 1] containing confidence scores 
                  for each detection.
...
def export_torchscript(model, im, file, optimize, prefix=colorstr("TorchScript:")):
    ...
    Args:
        ...
        im (torch.Tensor): A tensor representing the input image for the model, typically with shape (N, C, H, W).
    ...
    Raises:
        Exception: Logs the error and returns `(None, None)` if any issue arises during export.
    
    Notes:
        - The function leverages `torch.jit.trace` to trace the model using the provided input tensor (`im`).
        - Metadata such as input shape, stride, and class names are embedded in an extra file stored with the
          TorchScript model.
        - When `optimize` is `True`, the exported model is optimized for mobile using `optimize_for_mobile`.
    ...
    For more info, visit: https://ultralytics.com/.
    """
...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr("ONNX:")):
    ...
    Notes:
        Ensure `onnx`, `onnx-simplifier`, and suitable runtime packages are installed:
            ...
            ```
        This function also verifies the exported ONNX model and, if requested, simplifies it.
...
def export_paddle(model, im, file, metadata, prefix=colorstr("PaddlePaddle:")):
    ...
    Args:
        model (torch.nn.Module): The YOLOv3 model to be exported.
        im (torch.Tensor): A sample input tensor used for tracing the model, with shape (B, C, H, W).
    ...
    Returns:
        tuple[str, None]: A tuple containing the directory path where the PaddlePaddle model is saved, and `None`.
...
def export_coreml(model, im, file, int8, half, nms, prefix=colorstr("CoreML:")):
    ...
    Example:
        ...
        export_coreml(model, im, file, int8, half, nms)
        ```
    """
...
def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False, prefix=colorstr("TensorRT:")):
    ...
    Returns:
        tuple (Path, None): The output file path and None.
...
def export_saved_model(
    ...
    """
    Exports a YOLOv3 model to TensorFlow SavedModel format. The function includes options for post-processing like Non-Max Suppression (NMS) 
    and others.
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr("TensorFlow Lite:")):
    ...
    Args:
        ...
        nms (bool): Flag to include Non-Max Suppression (NMS) in the TFLite model.
        agnostic_nms (bool): Flag to apply class-agnostic NMS.
        prefix (str): Prefix string for log messages. Defaults to `colorstr("TensorFlow Lite:")`.
    
    Returns:
        (str, None): The file path to the saved TensorFlow Lite model and None.
    ...
    Examples:
        ...
        # Load Keras model and create a dummy input tensor
        keras_model = load_model('model.h5')
        im = torch.randn(1, 3, 640, 640)
    
        # Export to TensorFlow Lite
        export_tflite(keras_model, im, Path("model.tflite"), int8=True, data="data.yaml", nms=True, agnostic_nms=True)
        ```
    
    Notes:
        - TensorFlow and appropriate dependencies must be installed.
        - INT8 quantization needs representative data for model calibration.
        - For more details, refer to TensorFlow Lite Developer Guide: https://www.tensorflow.org/lite/guide
    
    See Also:
        `export_onnx()`, `export_saved_model()`
    ```
    """
    import tensorflow as tf
...
def export_edgetpu(file, prefix=colorstr("Edge TPU:")):
    ...
    Notes:
        The function checks if the Edge TPU compiler is installed, and attempts installation if it isn't found. The 
        export process includes performing INT8 quantization on the model to make it Edge TPU-compatible.
...
def pipeline_coreml(model, im, file, names, y, prefix=colorstr("CoreML Pipeline:")):
    """
    Pre-processes a YOLOv3 image input, runs CoreML model prediction, and applies Non-Maximum Suppression (NMS).
    
    Args:
        model (coremltools.models.MLModel): The pre-loaded CoreML model to use for making predictions.
        im (torch.Tensor): Input image tensor in BCHW (batch, channel, height, width) format.
        file (pathlib.Path): Path to save the processed CoreML model with .mlmodel extension.
        names (dict): Dictionary mapping class indices to class names.
        y (torch.Tensor): YOLO model output tensor for inference shape purposes.
        prefix (str): Optional log prefix, defaults to 'CoreML Pipeline:'.
    
    Returns:
        (pathlib.Path | None): File path for the saved CoreML model if successful; None otherwise.
    ...
    Example:
        ...
        y = torch.randn(1, 25200, 85)  # Example YOLO output tensor
...
def parse_opt(known=False):
    ...
    Returns:
        argparse.Namespace: Namespace object containing export configuration parameters.
            - data (str): Path to dataset configuration file (default: 'data/coco128.yaml').
            - weights (list of str): List of model weight file paths (default: 'yolov3-tiny.pt').
            - imgsz (list of int): Image dimensions (height, width), default is [640, 640].
            - batch_size (int): Batch size for inference (default: 1).
            - device (str): Device to use for export ('cpu', '0' for GPU).
            - half (bool): Export the model with FP16 precision.
            - inplace (bool): Set YOLOv3 Detect() inplace=True.
            - keras (bool): Use Keras for TensorFlow SavedModel export.
            - optimize (bool): Optimize TorchScript model for mobile.
            - int8 (bool): Apply INT8 quantization for CoreML, TensorFlow, and OpenVINO.
            - dynamic (bool): Enable dynamic axes for ONNX, TensorFlow, TensorRT models.
            - simplify (bool): Simplify ONNX model after export.
            - opset (int): ONNX opset version (default: 17).
            - verbose (bool): Enable verbose logging for TensorRT export.
            - workspace (int): Set workspace size for TensorRT in GB (default: 4).
            - nms (bool): Add Non-Maximum Suppression (NMS) to TensorFlow models.
            - agnostic_nms (bool): Enable class-agnostic NMS for TensorFlow models.
            - topk_per_class (int): Keep top-K per class for TensorFlow JSON model (default: 100).
            - topk_all (int): Keep top-K overall for TensorFlow JSON model (default: 100).
            - iou_thres (float): IOU threshold for TensorFlow JSON model (default: 0.45).
            - conf_thres (float): Confidence threshold for TensorFlow JSON model (default: 0.25).
            - include (list of str): List of formats to include in export ('torchscript', 'onnx', etc.).
    
    Notes:
        The function uses `argparse` to define and process command-line arguments for various model export formats,
        including TorchScript, ONNX, OpenVINO, TensorRT, and more. Users can configure options such as image size, 
        batch size, device, and thresholds for NMS.
    """
...
def main(opt):
    """
    Main function to execute the export of a YOLOv3 PyTorch model to different formats.
        ...
        Args:
            opt (argparse.Namespace): Parsed command line arguments containing export configurations such as:
                ...
                weights (Union[int, str]): Path(s) to model weights file(s).
                ...
                include (List[Union[int, str]]): List of formats to export, e.g., ['torchscript', 'onnx'].
    ...
    Note:
    - Ensure that any necessary packages are installed for specific export formats before running the function.
    - Refer to the examples provided in the docstring for usage.
    """
    for opt.weights in opt.weights if isinstance(opt.weights, list) else [opt.weights]:
        run(**vars(opt))
